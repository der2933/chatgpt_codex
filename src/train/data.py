...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources and sources["image"]:
            videos = None
...
class EvaluationDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        assert len(sources) == 2 , "no implementation for multi turn"
        ...
        input_ids = prompt_input_ids.to(torch.long).squeeze(0)
        labels = response_input_ids.to(torch.long).squeeze(0)
        
        # prompt_input_ids != 0 attention_mask set to not mask ??????????????????
    ...
    def get_qids(self, ):
        return [i["id"] for i in self.list_data_dict]

class DataCollatorForSupervisedDataset(object):
    """Collate examples for supervised fine-tuning."""

    def __init__(self, pad_token_id: int, image_token_id: int, padding_side: str = "left"):
    ...
    def __call__(self, examples):
        ...
        batch_modality_type = []
        batch_image_grid_counts = []
        batch_video_grid_counts = []
        ...
        for example in examples:
            ...
            if "pixel_values_videos" in keys:
                ...
                batch_video_grid_counts.append(int(example["video_grid_thw"].size(0)))
                batch_image_grid_counts.append(0)
                has_video = True
            elif "pixel_values" in keys:
                batch_pixel_values.append(example["pixel_values"])
                batch_image_thw.append(example["image_grid_thw"])
                batch_image_grid_counts.append(int(example["image_grid_thw"].size(0)))
                batch_video_grid_counts.append(0)
                has_image = True
            else:
                batch_image_grid_counts.append(0)
                batch_video_grid_counts.append(0)
        ...
        token_modality_type = pad_sequence(batch_token_modality_type, padding_side=self.padding_side, padding_value=0)
        ...
        data_dict = {
            ...
            'image_grid_counts': torch.tensor(batch_image_grid_counts, dtype=torch.long),
            'video_grid_counts': torch.tensor(batch_video_grid_counts, dtype=torch.long),
        }
...
def make_supervised_data_module(model_id, processor, data_args):
    ...
    eval_sft_dataset = SupervisedDataset(
