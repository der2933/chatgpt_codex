...
from models.modeling_qwen2_5_vl import Qwen2_5_VLCausalLMOutputWithPast
import torch
...
import models.modeling_qwen2_5_vl
...
def replace_qwen2_5_with_mixed_modality_forward(use_liger=True):
    if use_liger:
        models.modeling_qwen2_5_vl.Qwen2_5_VLForConditionalGeneration.forward = qwen2_5_mixed_modality_forward_with_flce
    else:
        models.modeling_qwen2_5_vl.Qwen2_5_VLForConditionalGeneration.forward = qwen2_5_mixed_modality_forward
...
def qwen2_5_mixed_modality_forward_with_flce(
    ...
    if self.training and (labels is not None):
        ...
        if labels is not None:
            ...
            loss = loss_fct(shift_logits, shift_labels)
    if self.training:
        expert_sim_loss = self.compute_global_similarity_loss()
        if expert_sim_loss and loss:
            loss = loss + expert_sim_loss
